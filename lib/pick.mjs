...
// distinct packs (never the same one twice in a roll) and can weight the draw
// by rating so higher-rated packs come up more often, if you want that.
...
export function filterPacks(packs, f = {}) {
  return packs.filter(p => {
    ...
    if (f.minRating != null && !(Number(p.rating) >= f.minRating)) return false;
    if (f.text) {
      const t = hay(f.text);
 ...
 * @param {boolean} [opts.weighted=false]  weight by rating (higher = likelier)
...
export function pickRandom(packs, opts = {}) {
  ...
  while (pool.length && out.length < count) {
    let idx;
    if (opts.weighted) {
      const weights = pool.map(p => Math.max(1, Number(p.rating) || 10));
