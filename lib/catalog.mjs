...
// that different pack lists use in the wild (note/notes/description,
// name/title, creator/author), normalizes them to one shape, throws away anything without a usable
...
// Rocket League's rank ladder, so difficulty filters read in order instead of
// in whatever order the files listed them. Unknown names go last, alphabetically.
export const DIFFICULTY_ORDER = ['Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond', 'Champion', 'Grand Champion', 'Supersonic Legend'];

export function sortDifficulties(list) {
  const rank = s => { const i = DIFFICULTY_ORDER.findIndex(d => d.toLowerCase() === String(s).toLowerCase()); return i < 0 ? DIFFICULTY_ORDER.length : i; };
  ...
  for (const f of files) {
    ...
    for (const p of c.packs) {
      ...
      if (existing) {
        ...
        // merge any extra tags so filters still find it, and fill in fields the
        // first file left empty (a rating or category only one list has).
        ...
        for (const k of ['rating', 'category', 'difficulty', 'creator', 'notes']) if (existing[k] == null && p[k] != null) existing[k] = p[k];
      } else {
